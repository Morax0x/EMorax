...
// ( 🌟 الدوال المساعدة المفقودة - تمت إضافتها هنا 🌟 )
...
client.checkAndAwardLevelRoles = async function(member, newLevel) {
    ...
    try {
        ...
        if (roleToAdd && roleToAdd.position < botMember.roles.highest.position) { await member.roles.add(roleToAdd); }
        if (rolesToRemove.length > 0) { try { await member.roles.remove(rolesToRemove); } catch (e) {} }
...
client.sendLevelUpMessage = async function(messageOrInteraction, member, newLevel, oldLevel, xpData) {
    ...
    try {
        ...
        if (!channelToSend) {
            if (messageOrInteraction && messageOrInteraction.channel) channelToSend = messageOrInteraction.channel;
            else return; 
        }
        ...
        } else { embed = new EmbedBuilder().setAuthor({ name: member.user.tag, iconURL: member.user.displayAvatarURL({ dynamic: true }) }).setColor("Random").setDescription(`**Congratulations** ${member}! You have now leveled up to **level ${newLevel}**`); }
        const perms = channelToSend.permissionsFor(guild.members.me);
        if (perms.has(PermissionsBitField.Flags.SendMessages) && perms.has(PermissionsBitField.Flags.ViewChannel)) { await channelToSend.send({ content: levelUpContent, embeds: [embed] }).catch(() => {}); }
...
client.sendQuestAnnouncement = async function(guild, member, quest, questType = 'achievement') {
    ...
    try {
        ...
        if (!notifSettings) { notifSettings = { id: id, userID: member.id, guildID: guild.id, dailyNotif: 1, weeklyNotif: 1, achievementsNotif: 1, levelNotif: 1 }; client.setQuestNotif.run(notifSettings); }
        ...
        if (!channel) return;
        const canAttachFiles = channel.permissionsFor(guild.members.me).has(PermissionsBitField.Flags.AttachFiles);
        const questName = quest.name;
        const reward = quest.reward; 
        let message = ''; let files = []; 
        ...
        if (canAttachFiles) { try { let attachment; if (questType === 'achievement') { attachment = await client.generateSingleAchievementAlert(member, quest); } else { const typeForAlert = questType === 'weekly' ? 'rare' : 'daily'; attachment = await client.generateQuestAlert(member, quest, typeForAlert); } if(attachment) files.push(attachment); } catch (imgErr) { console.error("[Image Gen Fail]", imgErr); } }
        if (questType === 'achievement') { message = [ `╭⭒★︰ ${client.EMOJI_WI} ${userIdentifier} ${client.EMOJI_WII}`, `✶ انـرت سمـاء الامـبراطـوريـة بإنجـازك ${client.EMOJI_FASTER}`, `✥ انـجـاز: **${questName}**`, ``, `- فـالتسـجل امبراطوريتـنـا اسمـك بيـن العضـمـاء ${client.EMOJI_PRAY}`, rewardDetails, panelChannelLink ].join('\n'); } else { const typeText = questType === 'daily' ? 'يوميـة' : 'اسبوعيـة'; message = [ `╭⭒★︰ ${client.EMOJI_WI} ${userIdentifier} ${client.EMOJI_WII}`, `✶ اتـممـت مهمـة ${typeText}`, `✥ الـمهـمـة: **${questName}**`, ``, `- لقـد أثبـت انـك احـد اركـان الامبراطـورية ${client.EMOJI_PRAY}`, `- لا يُكلـف مثـلك الا بالمستحيـل ${client.EMOJI_COOL} ~`, rewardDetails, panelChannelLink ].join('\n'); }
    ...
    for (const quest of questsToCheck) {
        ...
        if (currentProgress >= quest.goal) {
            ...
            if (!existingClaim) {
                ...
                if (levelData.xp >= nextXP) { const oldLevel = levelData.level; levelData.xp -= nextXP; levelData.level += 1; }
...
client.checkAchievements = async function(client, member, levelData, totalStatsData) {
    ...
    for (const ach of questsConfig.achievements) {
        ...
        else if (ach.stat === 'meow_count' || ach.stat === 'total_meow_count') { let ld = levelData || client.getLevel.get(member.id, member.guild.id); currentProgress = ld ? (ld.total_meow_count || 0) : 0; }
        else if (ach.stat === 'boost_count') { let ld = levelData || client.getLevel.get(member.id, member.guild.id); currentProgress = ld ? (ld.boost_count || 0) : 0; }
        ...
        else { if (['has_caesar_role', 'has_race_role', 'has_tree_role', 'has_tag_role'].includes(ach.stat)) continue; continue; }
        if (currentProgress >= ach.goal) {
            ...
            if (!existingAch) {
                ...
                if (ld.xp >= nextXP) { ld.xp -= nextXP; ld.level += 1; }
...
// ( 🌟 Pass antiRolesCache to handler 🌟 )
